(function ($) {
    ...
    easyrtc.connect(
      ...
      function (socketId) {
        ...
        easyrtc.webSocket.on("ui_user_add", function (userData) {
          elUsers.append(
            "<div id=" + userData.id + ">" + userData.name + "</div>"
          );
        });
        ...
        easyrtc.webSocket.on("ui_user_set", function (userList) {
          for (id in userList) {
            elUsers.append(
              "<div id=" + userList[id].id + ">" + userList[id].name + "</div>"
            );
          }
        });
  ...
  function addMessage(text, senderId) {
    ...
    if (!senderId) {
      ...
      elMessages.append(
        "<div>" +
          (senderId == self.id ? "<b>Me: </b>" : "<b>Stranger: </b>") +
          content +
          "</div>"
      );
    }
  ...
  function connectMeToPartner(id) {
    ...
    elMessages.html("");
    addMessage("You are now chatting with a random stranger. Say hi!");
    // set the partner
    self.partnerId = id;
  ...
  ////////mic
  var btnMute = $("#mute_btn");
  btnMute.on("click", function () {
    var audioTrack = easyrtc.getLocalStream().getAudioTracks()[0];
  ...
  //// toglle video shesh
  var btnToggle = $("#toggle_camera_btn");
  btnToggle.on("click", function () {
    var videoTracks = easyrtc.getLocalStream().getVideoTracks()[0];
    if (videoTracks.enabled) {
      videoTracks.enabled = false;
      btnToggle.html("Enable");
    } else {
      videoTracks.enabled = true;
      btnToggle.html("Disable");
    }
  });
